...
export class ClassificationService {
  ...
  async classifyAndStore(emailId: string, userId: string): Promise<Email | null> {
    ...
    // Short-circuit: skip if already classified to save Gemini API quota
    if (email.confidence !== undefined && email.confidence !== null) {
      return email;
    }
  ...
  async batchClassify(userId: string, emailIds: string[]): Promise<void> {
    // Run sequentially with a 1-second delay between operations to respect free-tier rate limits
